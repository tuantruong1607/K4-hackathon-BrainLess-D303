...
// ─── Upload Modal ─────────────────────────────────────────────
function UploadModal({ onClose, onAdd }: { onClose: () => void; onAdd: (d: { title: string; lesson: string; file: File }) => void }) {
...
export default function SlideManagement() {
  ...
  const { data: slidesResponse, refetch } = useQuery({
  ...
  const decksList: SlideDeck[] = useMemo(() => {
    if (!slidesResponse) return [];
    return slidesResponse.map((item: any, idx: number) => ({
      id: item.id,
      title: item.title,
      lesson: item.day,
      status: "published" as SlideStatus,
      slides: [
        { kicker: "PDF Document", title: item.title, accent: "Slide Deck", note: "Click to download or view" }
      ],
      uploadedAt: item.createdAt ? new Date(item.createdAt).toISOString().split("T")[0] : "",
      updatedAt: item.createdAt ? new Date(item.createdAt).toISOString().split("T")[0] : "",
      views: 120,
  ...
  const addMutation = useMutation({
    mutationFn: async ({ title, lesson, file }: { title: string; lesson: string; file: File }) => {
      const formData = new FormData();
      formData.append("title", title);
      formData.append("day", lesson);
      formData.append("pdf", file);

      await apiFetch("/slides", {
        method: "POST",
        body: formData,
      });
    },
    ...
    onError: (err: any) => {
      alert(err.message || "Failed to upload slides");
    },
  });
  ...
  const toggleStatus = (_id: string) => {
    // status is local for published/draft toggle
  };
  ...
  const stats = useMemo(() => ({
    total: decksList.length,
    published: decksList.filter((d) => d.status === "published").length,
    totalSlides: decksList.reduce((s, d) => s + d.slides.length, 0),
    totalViews: decksList.reduce((s, d) => s + d.views, 0),
  }), [decksList]);
